...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for symbol in basket:
            order_depth = state.order_depths[Symbol(symbol)]
            buy_orders = list(order_depth_gb.buy_orders.items())
            sell_orders = list(order_depth_gb.sell_orders.items())
            best_bid, best_bid_amount = list(state.order_depths[symbol].buy_orders.items())[0]
            best_ask, best_ask_amount = list(state.order_depths[symbol].sell_orders.items())[0]
