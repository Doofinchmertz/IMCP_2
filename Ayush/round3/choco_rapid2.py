...
class Trader:
    ...
    def orders_chocolate(self, state: TradingState):
        ...
        undercut_bid = best_bid_chocolate + 1
        undercut_ask = best_ask_chocolate - 1

        
        curr_pos = self.position["CHOCOLATE"]

        if len(self.choco_cache) == 3:
            diff1 = self.choco_cache[1] - self.choco_cache[0]
            diff2 = self.choco_cache[2] - self.choco_cache[1]
            ...
            # if(mid_price_chocolate > self.choco_cache[0]):
            #     vol = CHOCO_LIMIT - curr_pos
            #     orders.append(Order("CHOCOLATE", best_bid_chocolate, -vol))
            # elif(mid_price_chocolate < self.choco_cache[0]):
            #     vol = -CHOCO_LIMIT - curr_pos
            #     orders.append(Order("CHOCOLATE", best_ask_chocolate, -vol))
        ...
        #     if (choco_recent < avg_choco - 0.5*std_choco):
        #         print("hi_minus")
        #         vol = CHOCO_LIMIT - curr_pos
        #         orders.append(Order("CHOCOLATE", best_ask_chocolate, vol))
        #     elif (choco_recent > avg_choco + 0.5*std_choco):
        #         vol = -CHOCO_LIMIT - curr_pos
        #         print("hi_plus")
        #         orders.append(Order("CHOCOLATE", best_bid_chocolate, vol))
