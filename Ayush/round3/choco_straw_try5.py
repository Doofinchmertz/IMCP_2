...
class Trader:
    ...
    def orders_strawberries(self, state: TradingState):
        STRAW_LIMIT = 350
        orders: list[Order] = []
        ...
        avg_straw = np.mean(np.array(self.strawberries_cache))
        std_straw = np.std(np.array(self.strawberries_cache))

        curr_pos = self.position["STRAWBERRIES"]
        curr_pos_choco = self.position["CHOCOLATE"]

        # if (len(self.strawberries_cache) > 3):
        #     straw_recent = np.mean(np.array(self.strawberries_cache[-3:]))
        #     if (straw_recent < avg_straw - 1.5*std_straw):
        #         vol = STRAW_LIMIT - curr_pos
        #         logger.print("hi_minus")
        #         orders.append(Order("STRAWBERRIES", best_ask_strawberries, vol))
        #     elif (straw_recent > avg_straw + 1.5*std_straw):
        #         vol = -STRAW_LIMIT - curr_pos
        #         logger.print("hi_plus")
        #         orders.append(Order("STRAWBERRIES", best_bid_strawberries, vol))
        if curr_pos_choco < 0:
            vol = STRAW_LIMIT - curr_pos
            orders.append(Order("STRAWBERRIES", best_ask_strawberries, vol))
    ...
    def run(self, state: TradingState):
        ...
        result['STRAWBERRIES'] += self.orders_strawberries(state)
