...
class Trader:
    ...
    def orders_mm_orchids(self, order_depth):
        ...
        for ask, vol in sell_orders:
            if ((ask < undercut_buy) or ((self.position["ORCHIDS"] < 0) and (ask == undercut_buy))) and curr_pos < ORCHIDS_POS_LIMIT:
        ...
        for bid, vol in buy_orders:
            if ((bid > undercut_sell) or ((self.position["ORCHIDS"] > 0) and (bid == undercut_sell))) and curr_pos > -ORCHIDS_POS_LIMIT:
    ...
    def check_arbitrage_orchids(self, order_depth, state: TradingState):
        orders: list[Order] = []
    ...
    def run(self, state: TradingState):
        ...
        undercurrent_buy = best_bid + 1
        undercurrent_sell = best_ask - 1
        ...
        result["ORCHIDS"] += orders_mm_orchids(order_depth)
        conversions = check_arbitrage_orchids(order_depth, state)
