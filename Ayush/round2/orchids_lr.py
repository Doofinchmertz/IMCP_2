...
class Trader:
    ...
    def compute_orders_orchids(self, order_depth, acc_bid, acc_ask):
        ...
        for ask, vol in sell_orders:
            if ((ask <= acc_bid) or ((self.position["STARFRUIT"] < 0) and (ask == acc_bid + 1))) and curr_pos < ORCHIDS_POS_LIMIT: ## Try adding (self.position[product]<0) and (ask == acc_bid+1) to the condition
        ...
        for bid, vol in buy_orders:
            if ((bid >= acc_ask) or ((self.position["STARFRUIT"] > 0) and (bid == acc_ask - 1))) and curr_pos > -ORCHIDS_POS_LIMIT: ## Try adding (self.position[product]>0) and (bid == acc_ask-1) to the condition
