...
class Trader:
    ...
    def orders_mm_orchids(self, order_depth, ducks_price):
        ...
        num_bins = int(undercut_sell) - int(ducks_price) - 1
        
        if num_bins > 0:
            weights_bin = [0] * num_bins
            weights_bin[0] = 1

            for i in range(1, num_bins):
                weights_bin[i] = 0.9*weights_bin[i-1]

            ## normalise the weights:
            sum_weights = sum(weights_bin)
            weights_bin = [int((x/sum_weights)*(ORCHIDS_POS_LIMIT - curr_pos)) for x in weights_bin]
            volume_bins = weights_bin

            # num_of_orders = undercut_sell - ducks_price - 1
            # vol_of_orders = int((100 - curr_pos)/num_of_orders)
            i = 0
            for price in range(int(ducks_price) + 2, int(undercut_sell) + 1):
        ...
        ## for loop to place orders at all values above ducks_price and below undercut_sell
    ...
    def run(self, state: TradingState):
        ...
        result["ORCHIDS"] += self.orders_mm_orchids(order_depth, ducks_price_selling)

        curr_pos = self.position["ORCHIDS"]

        if undercut_sell > state.observations.conversionObservations["ORCHIDS"].askPrice + state.observations.conversionObservations["ORCHIDS"].importTariff + state.observations.conversionObservations["ORCHIDS"].transportFees:
            # orders.append(Order("ORCHIDS", best_ask, -best_ask_amount))
            conversions = -curr_pos
        
        curr_pos = self.position["ORCHIDS"]
        if undercut_buy < state.observations.conversionObservations["ORCHIDS"].bidPrice - state.observations.conversionObservations["ORCHIDS"].exportTariff - state.observations.conversionObservations["ORCHIDS"].transportFees -0.5:
            # orders.append(Order("ORCHIDS", best_bid, -best_bid_amount))
