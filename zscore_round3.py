...
# import pandas as pd
import json
from datamodel import Order, OrderDepth, ProsperityEncoder, TradingState, Symbol, Observation, Trade, Listing
...
##################################################################################
                ...
                if z_score_diff < -2:
                    ...
                    # print("SELL", "CHOCOLATE", 2 * limit_mult, "x", chocolate_best_bid)
                    ...
                    # print("SELL", "STRAWBERRIES", 4 * limit_mult, "x", strawberries_best_bid)
                    ...
                    # print("SELL", "CHOCOLATE", 2 * limit_mult, "x", chocolate_best_ask)
                    # orders_chocolate.append(Order("CHOCOLATE", chocolate_best_ask, 4 * limit_mult))
                    
                    # #dip_best_bid_vol = min(self.dip_limit-dip_pos, state.order_depths['DIP'].buy_orders[dip_best_bid])
                    # print("SELL", "STRAWBERRIES", 4 * limit_mult, "x", strawberries_best_ask)
                    # orders_strawberries.append(Order("STRAWBERRIES", strawberries_best_ask, 6 * limit_mult))
                    
                    # #ukulele_best_bid_vol = min(self.ukulele_limit-ukulele_pos, state.order_depths['UKULELE'].buy_orders[ukulele_best_bid])
                    # print("SELL", "ROSES", limit_mult, "x", roses_best_ask)
