...
class Trader:
    ...
    def orders_chocolate(self, state: TradingState):
        ...
        if curr_pos == 0:
            ...
            self.choco_trade_max = -1
            self.choco_trade_min = 1e9

        if (len(self.choco_cache) > 3):
            # choco_recent = np.mean(np.array(self.choco_cache[-3:]))
            spread_recent = np.mean(np.array(self.spread_cache[-3:]))

            if (spread_recent < avg_spread - 1.5*std_spread):
                print("hi_minus")
                vol = CHOCO_LIMIT - curr_pos
                orders.append(Order("CHOCOLATE", best_ask_chocolate, vol))
                self.trade_price_choco = best_ask_chocolate
                self.trade_price_max = best_ask_chocolate
                self.trade_price_min = best_ask_chocolate
                self.is_choco_trade = True
            elif (spread_recent > avg_spread + 1.5*std_spread):
                vol = -CHOCO_LIMIT - curr_pos
                print("hi_plus")
                orders.append(Order("CHOCOLATE", best_bid_chocolate, vol))
                self.trade_price_choco = best_bid_chocolate
                self.trade_price_max = best_bid_chocolate
                self.trade_price_min = best_bid_chocolate
                self.is_choco_trade = True
        
        if self.is_choco_trade == True:
            ## Checking for long or short

            self.choco_trade_max = max(self.trade_price_max, best_ask_chocolate)
            self.choco_trade_min = min(self.trade_price_min, best_ask_chocolate)
                
            if self.position['CHOCOLATE'] > 0:
            
                if best_ask_chocolate - self.trade_price_choco < -6 and self.choco_trade_max - best_ask_chocolate > 10:
                    orders.append(Order(symbol="CHOCOLATE", price=best_bid_chocolate, quantity = -self.position['CHOCOLATE']))
                # self.is_choco_trade = False

            if self.position['CHOCOLATE'] < 0:
                
                if best_ask_chocolate - self.trade_price_choco > 6 and best_ask_chocolate - self.choco_trade_min > 10:
                        orders.append(Order(symbol="CHOCOLATE", price=best_ask_chocolate, quantity = -self.position['CHOCOLATE']))

                # if best_ask_chocolate > self.trade_price_choco + self.std_choco:
    ...
    def run(self, state: TradingState):
        ...
        result["GIFT_BASKET"] += self.get_orders_basket(state)
